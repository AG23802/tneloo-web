...
export class ThreadView implements OnInit, OnDestroy {
  ...
  // different sources (thread-view, chat.service, touch events) can be
  // lined up on one timeline to pinpoint exactly when something happened.
  private log(message: string, data?: unknown): void {
    console.log(`[thread-view t=${performance.now().toFixed(1)}] ${message}`, data ?? '');
  }
  ...
  // Container-level touch tracking. Separate from the per-row swipe-to-
  // reveal-timestamp handlers below — this fires in addition to those via
  // bubbling, which is fine.
